...
from apple_health.report_models import ActivitySummary, DailySummary, MonthlySummary, SleepMonthlySummary, SleepScore
...
class TextRenderer:
    ...
    def _render_monthly_general_activity(self, writer: _TextWriter, summary: MonthlySummary) -> None:
        metrics = summary.activity_metrics
        if metrics is None or (metrics.total_steps is None and metrics.total_distance_km is None):
            return
        self._render_general_activity(writer, total_steps=metrics.total_steps, total_distance_km=metrics.total_distance_km, average_step_length_cm=metrics.average_step_length_cm, average_daily_steps=metrics.average_daily_steps, average_daily_distance_km=metrics.average_daily_distance_km)
        writer.write()
    ...
    def _render_monthly_weight(self, writer: _TextWriter, summary: MonthlySummary) -> None:
        ...
        writer.write(f'  Measurements:     {metrics.measurements:.0f}/{summary.reporting_days} days')
        writer.write()

    def _render_monthly_expenditures(self, writer: _TextWriter, summary: MonthlySummary) -> None:
        metrics = summary.activity_metrics
        if metrics is None or metrics.average_basal_energy_kcal is None or metrics.average_active_energy_kcal is None:
            return
    ...
    def _render_daily_general_activity(self, writer: _TextWriter, summary: DailySummary) -> None:
        self._render_general_activity(writer, total_steps=summary.total_steps, total_distance_km=summary.total_distance_km, average_step_length_cm=summary.average_step_length_cm)
        writer.write()
    ...
    def _render_activity(self, writer: _TextWriter, activity: ActivitySummary, reporting_days: int | None=None) -> None:
        ...
        if reporting_days is not None:
            divisor = activity.sessions if activity.activity_type in (WorkoutType.OUTDOOR_CYCLING, WorkoutType.INDOOR_CYCLING) else reporting_days
            averaging_label = 'Workout' if activity.activity_type in (WorkoutType.OUTDOOR_CYCLING, WorkoutType.INDOOR_CYCLING) else 'Daily'
            avg_duration = activity.duration_minutes / divisor
            avg_energy = activity.active_energy_kcal / divisor
            writer.write()
            writer.write(f'  Average {averaging_label} Duration: {self._format_minutes(avg_duration)}')
            writer.write(f'  Average {averaging_label} Energy:   {avg_energy:.0f} kcal')
            if activity.distance_km is not None:
                writer.write(f'  Average {averaging_label} Distance: {activity.distance_km / divisor:.2f} km')
        writer.write()

    def _render_general_activity(self, writer: _TextWriter, total_steps: int, total_distance_km: float, average_step_length_cm: float, average_daily_steps: float | None=None, average_daily_distance_km: float | None=None) -> None:
    ...
    def _render_daily_sleep(self, writer: _TextWriter, summary: DailySummary) -> None:
        ...
        writer.write(f'  Time asleep:      {self._format_minutes(sleep.time_asleep_minutes)} ({sleep.awake_minutes:.0f} min awake)')
    ...
    def _render_monthly_sleep_score(self, writer: _TextWriter, summary: SleepMonthlySummary) -> None:
        header = 'Sleep score'
        writer.write(header)
        writer.write('-' * len(header))
