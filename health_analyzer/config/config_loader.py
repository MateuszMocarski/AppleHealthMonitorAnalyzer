...
from health_analyzer.config.sleep_score_config import (
    BedtimeScoreConfig,
...
class ConfigLoader:
    ...
    def load(
        ...
        *,
        ...
        if health_analyzer_app_source is not None:
            config.source.health_analyzer_app_source = health_analyzer_app_source
    ...
    def _build_source_config(data: Any) -> SourceConfig:
        ...
        if "health_analyzer_app_source" in section:
            kwargs["health_analyzer_app_source"] = ConfigLoader._require_string(
                section["health_analyzer_app_source"],
                f"{path}.health_analyzer_app_source",
            )
