...
```text
connected_health/config/app_config.py
```
...
```text
connected_health/providers/apple/config.py
```
...
```text
connected_health/config/sleep_config.py
```
...
```text
connected_health/config/sleep_score_config.py
```
...
```text
connected_health/config/examples/
├── config.example.toml
...
```bash
python app.py import export.zip --config connected_health/config/examples/config.strict-schedule.toml
```
...
The built wheel includes the TOML files under `connected_health/config/examples/` because the browser serves `config.example.toml` at runtime. This README is repository documentation and is not required by the runtime package.

```text
connected_health/config/
├── __init__.py
├── app_config.py
├── config_loader.py
├── examples/
