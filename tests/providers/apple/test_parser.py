...
from health_analyzer.providers.apple.constants import (
...
from health_analyzer.exceptions import HealthDataParseError
...
def test_aggregates_nutrition_records() -> None:
    ...
    xml = _wrap_xml(
        _record(
            ...
            source_name=source_config.health_analyzer_app_source,
            value="1000",
        ),
        _record(
            record_type="HKQuantityTypeIdentifierDietaryEnergyConsumed",
...
def test_ignores_health_analyzer_metrics_from_wrong_source() -> None:
    data = _parse_xml(
        _wrap_xml(
            _record(
                record_type="HKQuantityTypeIdentifierDietaryProtein",
...
def test_uses_configured_health_analyzer_source() -> None:
    config = AppConfig()
    config.source.health_analyzer_app_source = "Custom Health"
...
def test_preserves_missing_activity_metrics_on_non_activity_day() -> None:
    ...
    xml = _wrap_xml(
        _record(
            record_type="HKQuantityTypeIdentifierDietaryEnergyConsumed",
            source_name=config.source.health_analyzer_app_source,
            value="2000",
        )
    )
